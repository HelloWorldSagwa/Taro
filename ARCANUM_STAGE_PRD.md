...
| Unity Editor 버전 | **Unity 6000.0.43f1 (Unity 6.0 LTS)**로 고정. 현재 로컬 Unity Hub에서 설치 가능한 버전을 기준으로 프로젝트 시작 |
| 프로젝트 구조 | Unity 클라이언트 저장소와 서버 저장소 분리 |
| Unity MCP | 선택 사항. 있으면 씬/프리팹 자동 조작에 활용하되, 필수 착수 조건은 아님 |
...
| Unity Editor 버전 | **결정: Unity 6000.0.43f1 (Unity 6.0 LTS)**. 현재 로컬 설치 가능 버전을 기준으로 프로젝트 고정. 추후 6000.3 LTS 설치가 가능해지면 별도 업그레이드 스프린트로 검토 |
| Unity MCP | **결정: 선택 사항**. 있으면 씬/프리팹 자동화에 사용하되 필수 조건은 아님 |
