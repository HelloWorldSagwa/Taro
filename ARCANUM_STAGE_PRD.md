...
문서 버전: v0.1  
작성일: 2026-05-18  
제품 유형: 100% 그래픽 기반 PC Standalone 타로 앱  
핵심 장르: 타로 + 인터랙티브 의식 + 감정형 캐주얼 게임  
주요 시장: 한국 PC 사용자 우선, 20~40대 여성 중심, 연애/재회/상대 마음 고관여층. 모바일은 후속 플랫폼으로 확장
...
| 공유성 | 텍스트 캡처 | PC 결과 이미지, 후속 세로형 SNS 이미지/짧은 영상 |
...
결과 화면은 텍스트 페이지가 아니라 카드가 놓인 무대이며, 좌측에는 타로 마스터가 크게 등장해 사용자를 직접 상담하는 느낌을 준다. Sprint 1 기본 레이아웃은 PC Standalone 16:9, 1920x1080 화면에 맞춘 JRPG 대화 화면 구조다.
...
| 카메라 | PC 16:9, 중앙 카드덱 중심, 마우스/포인터 조작에 반응 |
...
```mermaid
flowchart TD
    A["Art Bible / VisualStyleGuide"] --> B["Asset Brief"]
    B --> C["GPT-Image-2 Generation"]
    C --> D["Candidate Set 4-8 variants"]
    D --> E["Visual QA Review"]
    E --> F{Approved?}
    F -- "No" --> G["Prompt Revision / Regenerate"]
    G --> C
    F -- "Yes" --> H["Post-process / Crop / Layer Prep"]
    H --> K{"Needs transparency?"}
    K -- "Yes" --> L["Green Screen Keying / Alpha Cleanup"]
    K -- "No" --> I["Unity Addressables"]
    L --> I
    L --> J["CMS Asset Registry"]
    I --> J
```
...
| UI 호환성 | Unity 16:9 화면에서 핵심 피사체가 유지되고, 대사창/카드 UI를 가리지 않는다. |
...
### 7.6.1 메인/프로필/타로보기 화면 에셋 브리프

방금 생성된 GPT-Image-2 게임화면 방향성은 `고급 JRPG 오컬트 무대 + 반투명 유리 UI + 촛불 금박 + 장미 포인트 + 좌측 타로 마스터 + 한글 친화 대사창`으로 고정한다. 에셋은 화면 파이프라인별로 브리프를 작성하고, 실제 한글 문구는 Unity 텍스트 레이어에서만 처리한다.

| 파이프라인 | 필수 에셋 ID | 형식 | 생성/후처리 규칙 |
|---|---|---|---|
| 메인화면 | `BG_MAIN_STAGE_TABLE_16X9` | opaque PNG | 타로 테이블과 카드덱이 첫 화면의 중심. 좌측 캐릭터 영역과 하단 대사창 영역을 비워 둔다. |
| 메인화면 | `UI_LOGO_ARCANUM_STAGE_KO_FRAME`, `UI_FRAME_TOP_NAV_GLASS` | transparent PNG | 로고/메뉴 장식 프레임만 생성. `ARCANUM STAGE`와 한글 메뉴명은 Unity 텍스트. |
| 메인화면 | `UI_BUTTON_PRIMARY_GOLD_IDLE/HOVER`, `UI_BUTTON_SECONDARY_GLASS_IDLE/HOVER` | transparent PNG | 한글 버튼 라벨을 위한 넓은 빈 중앙 영역, 9-slice 가능 구조, green-screen alpha. |
| 메인화면 | `MASTER_LUNA_HOME_POSE_WELCOME_EXPR_CALM/SMILE` | transparent PNG | 좌측 반신, 환영/안내용 표정, 머리/눈/장신구/소매 실루엣 고정. |
| 프로필생성 | `BG_PROFILE_ATELIER_16X9` | opaque PNG | 상담실/아틀리에 배경. 우측/중앙 폼 패널과 좌측 루나 스프라이트 영역을 비워 둔다. |
| 프로필생성 | `UI_PANEL_PROFILE_FORM_9SLICE`, `UI_INPUT_NAMEPLATE_KO_9SLICE`, `UI_CHOICE_CHIP_GLASS_IDLE/HOVER/SELECTED`, `UI_AVATAR_FRAME_DEFAULT` | transparent PNG | 한글 이름 입력, 감정/관심사 칩, 아바타 프레임을 지원. 모든 문자는 런타임 텍스트. |
| 프로필생성 | `MASTER_LUNA_PROFILE_POSE_GUIDE_EXPR_CALM/FOCUS/REASSURANCE` | transparent PNG | 프로필 질문을 안내하는 부드러운 반신 컷. 입력창을 가리지 않도록 좌측 35-45%에 배치. |
| 타로보기 | `BG_RITUAL_DAILY_TABLE_16X9`, `BG_RESULT_DAILY_TABLE_16X9`, `BG_RITUAL_RELATIONSHIP_16X9`, `BG_RESULT_RELATIONSHIP_16X9` | opaque PNG | 하단 18-24%는 대사창 안전 영역, 중앙/우측은 카드 선택/결과 카드 영역. |
| 타로보기 | `CARD_BACK_DEFAULT`, `FRAME_TAROT_DEFAULT`, `UI_DIALOGUE_BOX_JRPG_9SLICE`, `UI_NAMEPLATE_MASTER_KO_9SLICE`, `UI_CHOICE_BUTTON_READING_IDLE/HOVER/SELECTED` | mixed | 카드명/번호/대사는 이미지에 굽지 않는다. 프레임류는 green-screen alpha로 생성 후 투명화. |
| 타로보기 | `MASTER_LUNA_READING_POSE_GUIDE_EXPR_CALM/SMILE/CONCERN/FOCUS/SURPRISE/REASSURANCE`, `FX_CARD_REVEAL_GOLD_SMOKE_SPRITE`, `FX_CANDLE_DUST_LOOP_SPRITE` | transparent PNG | 리딩 상태별 표정과 카드 공개 이펙트. green-screen alpha 후 초록 잔색 검수 필수. |

GPT-Image-2 프롬프트 공통 규칙:

- 컷아웃/프레임/이펙트: `solid chroma key green background #00FF00, no shadows on background, no green object parts`를 포함한다.
- 한글 UI 프레임: `blank text-safe Korean JRPG UI frame, no text, no letters, no glyphs`를 포함한다.
- 배경: `no text, no logo, no watermark`와 함께 UI 안전 영역을 명시한다.
- 캐릭터 루나: `moon-silver hair`, `crescent gold jewelry`, `black-violet sleeves`, `rose-gold tarot accents`를 반복해 표정/포즈 일관성을 유지한다.
- 네거티브 프롬프트: `text, letters, Hangul, roman numerals, numbers, watermark, signature, fake glyphs, green object parts, malformed hands, cropped face, noisy text area`.
...
| 공유 템플릿 | PC 결과 이미지, 후속 세로 이미지, 카드 포스터, 짧은 영상 | ShareArtifact | SNS 확산 |
...
### 11.0 Sprint 1 수직 슬라이스 고정 범위

Sprint 1은 MVP 전체가 아니라 **오늘의 카드 1장 수직 슬라이스**다. 제품 검증 목표는 PC Standalone 16:9 화면에서 사용자가 앱을 실행하고, 타로 테이블에 진입하고, 카드 한 장을 직접 뽑고, 하단 JRPG 대사창으로 해석을 받은 뒤 결과 화면까지 도달하는 것이다.

현재 repo 구현 상태 기준 Sprint 1 씬 체인은 `Boot -> HomeTable -> Ritual -> ReadingResult`로 고정한다. Unity 프로젝트는 `Taro/` 아래에 있으며 Sprint 1 관련 씬과 스크립트는 `Taro/Assets/Arcanum/` 경로를 기준으로 관리한다.

| 구분 | Sprint 1 결정 |
|---|---|
| 플랫폼 | PC Standalone 우선 |
| 화면 | 16:9, 1920x1080 기준. 1600x900, 1280x720 보조 확인 |
| 리딩 | 오늘의 카드 1장만 |
| 카드 | 프로토타입 6장 로컬 덱 |
| 씬 | Boot, HomeTable, Ritual, ReadingResult |
| UI | uGUI 기반 카드 보드 + 좌측 타로 마스터 + 하단 JRPG 대사창 |
| 해석 | 로컬/검수 템플릿 기반 today-card 결과 |
| 에셋 | placeholder 또는 `approved` 후보만 사용 |
| 테스트 | Sprint 1 smoke/EditMode 중심, 오늘의 카드 흐름 검증 |

Sprint 1에서 제외하는 것은 상대 마음 3장, 재회 가능성 5장, 선택 A/B, 연간운세, 공유 이미지 완성, 실결제/실광고, Live2D/Spine, 모바일 대응이다. 이 항목들은 6주 MVP 또는 후속 Phase 후보로만 다룬다.
...
| FR-202 | 시스템은 Sprint 1 기준 1920x1080 공유 이미지를 생성해야 하며, 세로 SNS 이미지는 후속 포맷으로 지원해야 한다. | P1 |
...
erDiagram
    ...
    USER ||--o{ READING : owns
    USER ||--o{ COLLECTION_ITEM : collects
    USER ||--o{ USAGE_HABIT : develops
    USER ||--o{ RELATIONSHIP_CONTEXT : has
    USER ||--o| MONETIZATION_PREFERENCE : has
    USER ||--o{ MASTER_AFFINITY : builds
    USER ||--o{ REMINDER : receives
    SESSION ||--|| SESSION_CONTEXT : has
    TAROT_MASTER ||--o{ READING : guides
    TAROT_MASTER ||--o{ MASTER_AFFINITY : earns
    READING ||--o{ DIALOGUE_SCENE : opens
    DIALOGUE_SCENE ||--o{ DIALOGUE_LINE : contains
    DIALOGUE_LINE }o--o| INTERPRETATION : narrates
    READING }o--o| SESSION_CONTEXT : contextualized_by
    READING }o--o| RELATIONSHIP_CONTEXT : relates_to
    READING ||--o{ ACTION_GUIDE : produces
    READING ||--o{ READING_FEEDBACK : receives
    ACTION_GUIDE ||--o{ REMINDER : schedules
    QUESTION }o--|| QUESTION_THEME : belongs_to
    QUESTION }o--o| FORECAST_PERIOD : targets
    QUESTION ||--o| READING : starts
    READING }o--|| SPREAD : uses
    READING ||--o| FORECAST_REPORT : creates
    FORECAST_REPORT ||--o{ REPORT_SECTION : contains
    FORECAST_REPORT }o--|| FORECAST_PERIOD : covers
    FORECAST_REPORT ||--o{ CALENDAR_SLOT : divides_into
    CALENDAR_SLOT }o--|| CARD_INSTANCE : contains
    READING ||--o{ CARD_INSTANCE : contains
    CARD_INSTANCE }o--|| TAROT_CARD : references
    CARD_INSTANCE }o--|| SPREAD_POSITION : occupies
    READING ||--|| INTERPRETATION : generates
    READING ||--o| VISUAL_DIRECTION : drives
    READING ||--o{ SHARE_ARTIFACT : creates
    SHARE_ARTIFACT }o--|| SHARE_TEMPLATE : uses
    INTERPRETATION }o--o| PRODUCT : gated_by
    INTERPRETATION }o--o{ SAFETY_POLICY : constrained_by
    REWARD }o--o| COLLECTION_PROGRESS : advances
```
...
ARCANUM STAGE는 일반 앱 UI보다 게임형 그래픽, 카드 조작, 캐릭터 연출, 파티클, 사운드, 리소스 번들 관리가 핵심이다. 따라서 Sprint 1 클라이언트는 **Unity + C# 기반 PC Standalone**으로 확정하고, 모바일은 후속 플랫폼으로 확장한다.

| 영역 | 선택 | 이유 |
|---|---|---|
| PC Standalone 클라이언트 | Unity, C# | 카드 셔플/플립, 타로 마스터 캐릭터, 파티클, 사운드, Addressables 관리에 적합 |
...
| UI | uGUI | JRPG식 대화창, 카드 보드, PC 16:9 UI, 애니메이션 연출에 적합 |
...
| 후보 | 판단 |
|---|---|
| Unity | 채택. 게임형 카드 조작, 2D 캐릭터, 파티클, 사운드, PC Standalone 빌드와 후속 모바일 빌드 대응이 강하다. |
| Unreal | 과함. 고품질 3D에는 강하지만 2D 그래픽 타로 앱에는 빌드 크기와 개발 비용이 부담이다. |
...
```mermaid
flowchart TD
    U["User"] --> C["Unity PC Standalone Client"]
    C --> A["API Gateway / NestJS"]
    C --> CDN["Asset CDN"]
    C --> IAP["App Store / Play Store IAP"]
    C --> ADS["Ad Network SDK"]

    A --> DB["PostgreSQL"]
    A --> R["Redis"]
    A --> S3["Object Storage"]
    A --> AI["AI / Rule Engine"]
    A --> LOG["Event Pipeline"]

    CMS["Next.js Admin / CMS"] --> A
    CMS --> S3
    LOG --> BI["Analytics Dashboard"]
```
...
sequenceDiagram
    ...
    User->>Client: 테마/감정 선택
    Client->>API: createReading(SessionContext, Emotion, Theme)
    API->>DB: User/Relationship/UsageHabit 조회
    API->>CMS: 카드/해석/대사 템플릿 조회
    API->>API: SafetyPolicy + Rule Engine 적용
    API->>DB: Reading/CardInstance/Interpretation 저장
    API-->>Client: 카드 결과, DialogueLine, VisualDirection 반환
    Client-->>User: 카드 플립 + 타로 마스터 상담 연출
    User->>Client: 저장/공유/구매/광고 선택
    Client->>API: event log + state update
```
...
| 영역 | MVP 범위 |
|---|---|
| 클라이언트 | Unity PC Standalone 16:9 앱, uGUI, 카드 셔플/플립, 타로 마스터 1명, JRPG식 대화 UI |
...
| Unity 씬 | 타로 테이블, 카드 보드, 타로 마스터 레이어, UI 캔버스 구성 | PC 16:9 1920x1080 화면에서 겹침/잘림 없음 |
...
| 검증 방식 | Unity batchmode compile/test, 스크린샷 QA, PC 16:9 해상도별 검수 |
...
| Performance Gate | PC Standalone 1920x1080 기준 카드 플립/대사 UI가 끊김 없이 동작 |
...
| 구분 | 판단 |
|---|---|
| Unity로 구현 가능 | 카드 셔플/플립, JRPG 대화 UI, 타로 마스터 Live2D/Spine 연출, 파티클, PC Standalone 빌드, 후속 모바일 빌드 |
| 100% AI 작업 가능 | 코드 생성, 서버 구축, PRD 관리, 그래픽 생성/평가, 테스트 작성, QA 리포트 작성 |
| 품질 확보 조건 | 작은 범위로 만들고, 매번 실행/스크린샷/검수/수정 루프를 돌려야 함 |
| 가장 큰 리스크 | 그래픽 일관성, Unity 씬/프리팹 복잡도, 16:9 화면 구성 품질, Live2D/Spine 리깅 품질 |
...
목표: Sprint 1 오늘의 카드 수직 슬라이스로 카드 조작감과 비주얼 콘셉트 검증
...
| 이슈 | 결정 필요 |
|---|---|
| Sprint 1 범위 | **결정: 오늘의 카드 1장 수직 슬라이스만 검증**. `Boot -> HomeTable -> Ritual -> ReadingResult` 씬 체인을 PC Standalone 16:9에서 통과시키는 것이 목표 |
