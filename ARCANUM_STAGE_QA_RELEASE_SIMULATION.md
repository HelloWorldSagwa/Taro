...
2. GPT-Image-2 생성 에셋은 스타일 일관성, 왜곡, 투명화, 모바일 크롭 검수가 필수다.
...
### Week 3. 모바일 시각 QA와 수익화 mock

목표: 모바일 화면 품질과 mock 결제/광고 흐름을 릴리즈 차단 수준으로 끌어올린다.

| 영역 | 시뮬레이션 작업 | 테스트 데이터 | 통과 기준 |
|---|---|---|---|
| 해상도 매트릭스 | 360x780, 390x844, 412x915, 430x932, 768x1024 캡처 | 홈, 셔플, 결과, CTA, 공유 | 겹침/잘림 P0 0개, P1 0개 |
...
- Go: 모바일 스크린샷 기반 QA에서 화면별 P0/P1 결함이 없고, mock 수익화 상태 전이가 안정적이다.
...
| 해상도별 레이아웃 | 지정 해상도 batch screenshot | 버튼/대사/카드가 safe area 밖으로 이탈 |
...
- 360x780: 작은 Android 기준
- 390x844: iPhone 일반 기준
- 412x915: Android 일반 기준
- 430x932: 큰 iPhone 기준
- 768x1024: 태블릿 세로 기준
...
| Mobile Crop | 모바일 세로 화면에서 핵심 피사체가 UI에 가리지 않는다 |
...
| 모바일 safe area | 대사창, 카드, 캐릭터가 작은 화면에서 충돌 | 해상도 matrix를 PR 차단 조건으로 운영 |
...
| Visual Gate | 필수 모바일 해상도에서 P0/P1 시각 결함 0개 |
...
- 모바일 주요 해상도에서 대사창/카드/캐릭터가 읽히지 않거나 겹친다.
...
2. uGUI 기준으로 모바일 세로형 safe area와 JRPG 대사창 규격을 먼저 잠근다.
...
| UI Gate | 타로 마스터 좌측 대형 캐릭터, 중앙 카드, 하단 대사창이 9:16 화면에서 겹침 |
