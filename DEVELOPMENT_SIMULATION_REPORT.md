...
| 빌드 | Android 우선, iOS는 후속 |

uGUI를 권장하는 이유는 모바일 2D 게임식 UI, 대사창, 카드 배치, Canvas 기반 레이아웃 검증이 빠르기 때문이다. UI Toolkit은 도구형 앱에는 좋지만, 이번 앱의 핵심은 게임형 연출이다.
...
| QA | 360x780~768x1024 해상도 스크린샷 QA |
...
| 역할 | 작업 |
|---|---|
| Unity | `ShareRenderer`로 1080x1920 이미지 생성 |
...
| 역할 | 작업 |
|---|---|
| Unity | Android 개발 빌드, Addressables 정리, 성능 튜닝 |
...
| Performance | 저사양 Android 기준 30fps 방어 |
...
| Visual | 360x780, 390x844, 412x915, 430x932, 768x1024 |
...
ARCANUM STAGE는 “타로 앱”이라기보다 **타로 상담을 게임 장면으로 만든 모바일 인터랙티브 콘텐츠**로 잡는 것이 맞다.
